...
#let project(
  ...
  let font-default = (font: "Arial", lang: language, region: "ch", size: font-size)
  ...
  show heading.where(level: 1): h => {
    ...
    line(length: 100%, stroke: 0.18em + colors.violett)
    upper(h)
    v(0.45em)
  }

  show heading.where(level: 2): h => {
    set text(..font-special, size: 0.9em)
    upper(h)
  }
...
// "Definition"-Vorlage
#let definition(t) = {
  rect(stroke: 0.13em + colors.violett, inset: 0.73em, columns(1, t), width: 100%)
}
